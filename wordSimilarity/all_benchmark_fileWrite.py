    ...
    for i, filename in enumerate(os.listdir(word_sim_dir)):
        ...
        for line in open(os.path.join(word_sim_dir, filename), 'r'):
            ...
            if c > 0:
                ...
                for k in word_vec1:
                    for j in word_vec2:
                        sim_val = cosine_sim(word_vec1[k], word_vec2[j])
                        if diff == 1:
        ...
        file.write("\n")
        file.write(filename + "\t" + str(total_size) + "\t" + str(total_size - not_found) + "\t" + str(spearmans_rho(assign_ranks(manual_dict), assign_ranks(auto_dict))))
    file.close()
